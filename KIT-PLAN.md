...
| Trait composition | Emergent patterns with Certainty scoring | No hard constraints on which traits a model can mix. Common patterns emerge naturally and are documented with Certainty (confidence/likelihood). Outliers can deviate — the system doesn't block them, it makes the deviation visible. Most recipes are tacos; some are choco tacos. |
